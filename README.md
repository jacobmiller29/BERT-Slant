...
The slant method was pioneered by Gentzkow and Shapiro (2010). The idea is to identify ideological language that politicians use, and then to capture the use of this language in the media. For example (see Djourelova 2021), when disucssing the topic of immigration to the U.S. via non-legal routes, conservatives tend to refer to these immigrants as "illegal immigrants", while liberals tend to use the term "undocumented immigrants". Thus if a media outlet always uses the term "illegal immigrant" when discussing this topic, then it is probably a more conservative media outlet. 
